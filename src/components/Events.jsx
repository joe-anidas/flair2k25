    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          entry.target.classList.remove('translate-x-[-100%]', 'translate-x-[100%]', 'opacity-0', 'scale-95');
          // Add final classes with transition
          entry.target.classList.add('translate-x-0', 'opacity-100', 'scale-100');
        }
      });
    }, observerOptions);
          ...
          <div className="space-y-8 sm:space-y-10 lg:space-y-12">
            {events.map((event, index) => (
              <div
                ...
                className={`w-full sm:w-4/5 mx-auto transform transition-all duration-1000 ease-out will-change-transform ${
                  index % 2 === 0 
